...
export function VehicleFeatures({ features }: VehicleFeaturesProps) {
  ...
  const featureCategories = {
    comfort: ['Air Conditioning', 'Heated Seats', 'Leather Seats', 'Premium Sound', 'Sunroof', 'Tinted Windows'],
    safety: ['ABS', 'Airbags', 'Backup Camera', 'Blind Spot Monitoring', 'Lane Departure Warning', 'Parking Sensors'],
    technology: ['Apple CarPlay', 'Android Auto', 'Bluetooth', 'GPS Navigation', 'USB Charger', 'Wireless Charging'],
    convenience: ['Cruise Control', 'Keyless Entry', 'Power Locks', 'Power Windows', 'Remote Start', 'Automatic Headlights']
  };
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <svg className="w-5 h-5 text-zemo-yellow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-2">
            {categorizedFeatures.comfort?.map((feature, index) => (
              <li key={index} className={`flex items-center gap-2 ${feature.available ? 'text-gray-900' : 'text-gray-400'}`}>
                {feature.available ? (

...
export async function POST(
  ...
  try {
    ...
    if (!isHost && !isRenter) {
      return NextResponse.json({ error: 'Not authorized to perform pickup for this booking' }, { status: 403 });
    }
    ...
    for (const damage of damages) {
      ...
      if (damage.estimatedCost) {
        ...
        switch (damage.category) {
          case 'scratch':
            estimatedCost = damage.severity === 'minor' ? 50 : damage.severity === 'moderate' ? 150 : 400;
            break;
          case 'dent':
            estimatedCost = damage.severity === 'minor' ? 100 : damage.severity === 'moderate' ? 300 : 800;
            break;
          case 'crack':
            estimatedCost = damage.severity === 'minor' ? 80 : damage.severity === 'moderate' ? 250 : 600;
            break;
          case 'missing_part':
            estimatedCost = damage.severity === 'minor' ? 200 : damage.severity === 'moderate' ? 500 : 1000;
            break;
          case 'stain':
            estimatedCost = damage.severity === 'minor' ? 30 : damage.severity === 'moderate' ? 80 : 200;
            break;
          case 'mechanical':
            estimatedCost = damage.severity === 'minor' ? 300 : damage.severity === 'moderate' ? 800 : 2000;
            break;
          case 'electrical':
            estimatedCost = damage.severity === 'minor' ? 200 : damage.severity === 'moderate' ? 600 : 1500;
            break;
          default:
            estimatedCost = damage.severity === 'minor' ? 100 : damage.severity === 'moderate' ? 250 : 500;
        }
    ...
    const pickup = await prisma.vehicleInspection.create({
      data: {
        ...
        overallCondition: totalDamageScore === 0 ? 'EXCELLENT' : 
                         totalDamageScore <= 2 ? 'GOOD' :
                         totalDamageScore <= 5 ? 'FAIR' : 
                         totalDamageScore <= 10 ? 'POOR' : 'DAMAGED',
...
export async function GET(
  ...
  try {
    ...
    if (!isHost && !isRenter) {
      return NextResponse.json({ error: 'Not authorized to view this pickup inspection' }, { status: 403 });
    }

...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const agreementData = {
      hostName:
        `${booking.vehicle.host.profile?.firstName || ''} ${booking.vehicle.host.profile?.lastName || ''}`.trim() ||
        booking.vehicle.host.email,
    ...
    return NextResponse.json({ error: 'Failed to generate rental agreement' }, { status: 500 });
  }
}

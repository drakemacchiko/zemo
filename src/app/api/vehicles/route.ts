...
async function handleGet(request: AuthenticatedRequest) {
  try {
    ...
    const queryParams = {
      ...
      minSeatingCapacity: searchQuery.minSeatingCapacity
        ? parseInt(searchQuery.minSeatingCapacity)
        : undefined,
      locationLatitude: searchQuery.locationLatitude
        ? parseFloat(searchQuery.locationLatitude)
        : undefined,
      locationLongitude: searchQuery.locationLongitude
        ? parseFloat(searchQuery.locationLongitude)
        : undefined,
...
async function handlePost(request: AuthenticatedRequest) {
  try {
    ...
    if (!allowUnverified) {
      ...
      if (
        !userWithLicense?.drivingLicense ||
        userWithLicense.drivingLicense.verificationStatus !== 'VERIFIED'
      ) {
        return NextResponse.json(
      ...
      console.warn(
        'ALLOW_UNVERIFIED_LISTING is enabled — skipping driving license verification for vehicle creation'
      );
    }

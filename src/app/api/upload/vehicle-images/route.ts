...
async function handlePost(request: AuthenticatedRequest) {
  try {
    ...
    if (files.length > 20) {
      return NextResponse.json({ error: 'Maximum 20 photos allowed per upload' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Failed to upload vehicle images' }, { status: 500 });
  }
}

...
export async function GET(request: NextRequest) {
  try {
    ...
    const activities = recentBookings.map(booking => {
      ...
      const renterName = `${booking.user.profile?.firstName || ''} ${booking.user.profile?.lastName || ''}`.trim()

      let type: 'booking_request' | 'booking_completed' | 'review' | 'message' | 'payout' = 'booking_request'
      let title = ''
      let description = ''
